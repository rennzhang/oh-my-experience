...
| 字段 | 说明 | 示例 |
|------|------|------|
| 问题 | 上次哪里翻车了 | UI 任务未在真实浏览器验证就报告完成 |
| 反模式 | 要避免的做法 | 只在控制台/截图里看效果就说做完了 |
| 正确做法 | 应该怎么做 | 启动真实浏览器、检查响应式、交互、loading、error 和控制台 |
| 触发器 | 什么任务该召回 | UI 修改、前端修复、页面样式调整 |
| 抑制条件 | 什么情况不触发 | 纯后端改动、数据库迁移、配置修改 |
...
> 帮我执行审核结果：
> 1. `ome reflect apply RUN_ID --dry-run` 预览
> 2. 确认无误后 `ome reflect apply RUN_ID`
> 3. 对需要设为 active 的卡片，逐个 `ome experience promote DRAFT_ID`

入库是两步设计，Applica 之后卡片是 draft 状态，promote 之后才是 active。中间
你可以随时检查和修改 draft。
...
| 操作 | 什么时候用 | 命令 |
|------|-----------|------|
| approve | 这条经验可复用、触发条件精确 | `ome reflect decide RUN_ID CANDIDATE_ID --action approve` |
...
- **修正误触发。** 如果某张卡经常在不相关的任务中被召回，加 negative triggers 或收窄 tags。
...
每条候选必须包含：问题、反模式、正确做法、触发条件、抑制条件。
