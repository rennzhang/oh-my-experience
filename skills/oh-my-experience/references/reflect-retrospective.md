...
- **召回卡不当新证据**：复盘扫描任务可能触发 OME active 卡，这是执行护栏或去重线索，不是候选卡证据。主题相关旧卡只写入 `activeCardOverlapQa`；最终回复里的 `本次使用 N条 OME 经验卡` 只允许列出本轮实际采用的流程治理卡，不列主题旧卡。
...
Active 卡重叠检查只回答“是否已有相邻经验、应新建/合并/拒绝/改写哪张卡”。不要把
active 卡当成候选卡证据，也不要因为某张主题卡在扫描提示中被 hook 召回，就在最终
`本次使用 N条 OME 经验卡` footer 中声明使用了它。只有确实改变本轮复盘执行方式的流程
治理卡，才算本轮 used card。
...
用户确认入库后，完成标准不是只看到 active 卡文件，而是这张卡能被真实任务入口召回。

执行要求：

1. 先走正常生命周期：`ome reflect decide` -> `ome reflect apply --dry-run` -> `ome reflect apply` -> `ome experience enable`。
2. 启用后至少用 1 条、复杂或高风险卡用 2-3 条自然用户话术运行 `ome match "<真实任务入口>" --json`。
3. 召回话术优先来自 `criteria.use_when`、用户原话、典型未来任务，而不是卡片标题的机械复制。
4. 如果 `matches` 为空或命中了不该命中的卡，先检查 `criteria.use_when`、`criteria.ignore_when`、`recall.triggers`、`engine_hints`、`requiredSignals` 和 `blockedSignals`，再决定是否重写候选、归档错误 active 版本或调整 matcher。
5. `engine_hints.positive` / `requiredSignals` 只能用于强路由门槛。加了以后必须验证 matcher 的 `ruleSignals` 会在真实话术下产出对应 signal；否则应移除该正向门槛，保留自然语言触发和负向边界。
6. 只有 `active`、`ome experience show <id> --section rule` 可读、真实 `ome match` 可召回、错误旧版本不留在 active，才算经验入库完成。
