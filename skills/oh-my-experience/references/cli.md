...
- 运行测试、eval 或 smoke 时优先隔离 dataDir，不污染真实经验库。
...
`--index`，避免把 rule/body/sourceRefs 等重字段拉进上下文。
...
- Hook runtime smoke 应走真实 `ome hook run` 或项目测试，不再暴露独立 `eval hook` 命令。
...
- Hook 运行时会从 prompt payload 推导项目上下文，并应用卡片 `applicability`。
