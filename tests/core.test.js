...
test("hook context is neutral and carries scope hints", () => {
  ...
  assert.match(context, /Matched cards are optional reminders, not required reuse/);
  assert.match(context, /- Choice: You may apply a whole card, use only the useful parts, or ignore any match that does not fit the task/);
  assert.match(context, /- Before acting: If a card helps, say one short sentence about what OME reminded you to consider, then proceed/);
  assert.match(context, /- Final: If any card was used, state how many cards were used and include only the applied `Final link if used` values/);
  assert.doesNotMatch(context, /Retrospectives\/source scans/);
...
test("candidate links include all rendered cards without claiming usage", () => {
  ...
  assert.match(context, /state how many cards were used and include only the applied `Final link if used` values/);
